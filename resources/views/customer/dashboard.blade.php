...
<body class="bg-gray-100">
    <div class="flex h-screen">
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="flex items-center justify-between bg-white/80 backdrop-blur-sm rounded-xl p-4 shadow-sm border border-blue-200">
                                    ...
                                    @if($daysRemaining > 0 && $daysRemaining <= 7)
                                        <div class="w-2.5 h-2.5 bg-yellow-500 rounded-full animate-pulse shadow-md">
                                </div>
        ...
        {{-- === [BARU] ROW: KELAS SAYA === --}}
        <div class="w-full bg-white p-6 rounded-2xl shadow-md border border-gray-100 mb-6">
            ...
            {{-- Pastikan variable $myClasses dikirim dari controller --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-gray-50 rounded-xl p-5 border border-gray-200 hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <h4 class="font-bold text-gray-900 text-lg">{{ $class->gymClass->type ?? 'Class Name' }}</h4>
                            <p class="text-sm text-gray-600 flex items-center mt-1">
                                <i class="fas fa-user-circle mr-1.5 text-gray-400"></i>
                                {{ $class->gymClass->trainer->name ?? 'Trainer' }}
                            </p>
                        </div>
                        <span class="bg-white border border-gray-200 text-gray-600 text-xs font-bold px-2 py-1 rounded-md">
                            {{ \Carbon\Carbon::parse($class->gymClass->schedule ?? now())->format('H:i') }}
                        </span>
                    </div>

                    <div class="text-xs text-gray-500 mb-4 flex items-center">
                        <i class="far fa-calendar-alt mr-1.5"></i>
                        {{ \Carbon\Carbon::parse($class->gymClass->schedule ?? now())->format('d M Y') }}
                    </div>

                    {{-- [KODE TOMBOL ANDA DISINI] --}}
            ...
            @else
            <div class="text-center py-8">
                <p class="text-gray-500 italic">Anda belum terdaftar di kelas manapun.</p>
            </div>
            @endif
        </div>
