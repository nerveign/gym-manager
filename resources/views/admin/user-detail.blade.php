        ...
        <div class="flex-1 ml-64 h-screen flex flex-col">
            ...
            <div class="bg-white border-b px-8 py-4 flex justify-between items-center shadow-sm shrink-0 z-20">
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.users_management') }}" class="w-9 h-9 flex items-center justify-center bg-white border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 hover:text-indigo-600 transition shadow-sm">
                        <i class="fas fa-arrow-left text-sm"></i>
                    </a>
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 leading-tight">{{ $customer->name }}</h1>
                        <div class="flex items-center gap-2 text-xs text-gray-500">
                            <span>#USER-{{ str_pad($customer->id, 4, '0', STR_PAD_LEFT) }}</span>
                            <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                            <span>{{ ucfirst($customer->role) }}</span>
